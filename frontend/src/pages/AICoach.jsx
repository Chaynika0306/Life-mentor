...
function AICoach() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Add AI response as structured message
        setMessages((prev) => [
          ...prev,
          {
            from: "ai",
        ...
        setMessages((prev) => [
          ...prev,
          { from: "ai", text: data.message || "Something went wrong. Please try again." },
        ]);
      }
    } catch (err) {
      setMessages((prev) => [
        ...prev,
        { from: "ai", text: "I'm having trouble connecting right now. Please try again in a moment. 🙏" },
      ]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="ai-page">
      ...
      <div className="ai-header">
        ...
        <div className="ai-header-right">
          ...
          >
            {showHistory ? "💬 Chat" : "📊 Mood History"}
          </button>
          <button className="secondary-btn" onClick={() => navigate("/dashboard-home")}>
            ← Back
          </button>
        </div>
      </div>
                  ...
                  {msg.support && (
                    <div className="ai-response-card" style={{ background: moodColor[msg.mood] || "#d8ede8" }}>
                      ...
                      <p className="ai-counsellor-note">
                        💚 If you'd like deeper support, our counsellor is here for you.{" "}
                        <span
            ...
            <div className="ai-history-grid">
              {history.map((entry, i) => (
                <div
                  ...
                  style={{ borderLeft: `4px solid ${moodColor[entry.mood] || "#6aab99"}` }}
                >
                  <div className="ai-history-top">
                    <span className="ai-history-mood">
